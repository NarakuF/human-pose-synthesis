...
if __name__ == "__main__":
    ...
    data_dir = 'd:/data/In-shop Clothes Retrieval Benchmark/Eval'

    i = 10
    df = pd.read_csv('../imgs/keypoints.csv', sep=':')

    for index, row in df.iterrows():
        pose_cords = load_pose_cords_from_strings(row['keypoints_y'], row['keypoints_x'])

        colors, mask = draw_pose_from_cords(pose_cords, (256, 256))

        mmm = produce_ma_mask(pose_cords, (256, 256)).astype(float)[..., np.newaxis].repeat(3, axis=-1)
        print(mmm.shape)
        img = imread('../imgs/' + row['name'])
